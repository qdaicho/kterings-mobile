...
// import NoNotifcations from "@assets/images/no_search.svg";
// Sample JSON data for search
const sampleSearch: any[] = [
  ...
  // Add more sample search as needed
];
...
export default function Search() {
  ...
  useEffect(() => {
    // Simulate loading search from JSON data initially
    setSearch(sampleSearch.map(search => ({ ...search, read: false })));
  ...
  const handleNotificationClick = (id: number) => {
    const updatedSearch = search.map(search =>
      search.id === id ? { ...search, read: true } : search
    );
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={{ fontSize: 18, fontFamily: 'TT Chocolates Trial Bold', color: '#000000', marginTop: 120, marginBottom: 20 }}>Search</Text>
      ...
      {search.length > 0 ? (
        ...
        <View style={{ flex: 1,alignItems: 'center', marginTop: 150 }}>
          ...
          <Text style={{ fontSize: 18, fontFamily: 'TT Chocolates Trial Bold', color: '#000000', marginTop: 20 }}>No Search</Text>
        </View>
      )}
    </View>
  );
